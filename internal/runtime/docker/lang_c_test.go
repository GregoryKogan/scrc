...
func TestCStrategyPrepareBuildFailure(t *testing.T) {
	t.Parallel()
...
func TestCStrategyPrepareSuccess(t *testing.T) {
	t.Parallel()
	...
	client.onCreate(func(id string) {
		...
		client.setInspect(id, types.ContainerJSON{ContainerJSONBase: &types.ContainerJSONBase{State: &types.ContainerState{}}})
		client.setLogs(id, "", "")
		client.setCopyFrom(id, path.Join(runtime.config.Workdir, cBinaryFilename), newTarArchive(t, cBinaryFilename, binary))
	})
	...
	prep, ok := prepared.(*cPreparedScript)
	if !ok {
		t.Fatalf("expected cPreparedScript type")
	}
...
func TestCPreparedScriptRunPropagatesError(t *testing.T) {
	t.Parallel()
