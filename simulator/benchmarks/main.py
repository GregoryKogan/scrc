    ...
    for matrix in plan:
        ...
        for case in matrix.cases:
            if matrix.chart_type in {"line", "heatmap"}:
                ...
                case_key = case.name
            LOGGER.info(
                "Running case %s (load=%s, replicas=%d, parallel=%d)",
                case.name,
            ...
            with runner_manager.run(
                ...
                collector = BenchmarkResultCollector(
                    ...
                    group_id=f"{args.group_id}-{case.name}",
                ...
                collector.wait_for_pending(timeout_s=60.0)
                collector.stop()

                df = collector.build_dataframe()
                matrix_results[case_key] = df
                throughput_summary[case_key] = stats.throughput_per_minute

                df_path = output_dir / f"{matrix.label}__{case.name}.csv"
                df.to_csv(df_path, index=False)
                LOGGER.info(
                    "Saved raw results to %s (%d rows, throughput %.2f submissions/min)",
                    df_path,
                    len(df),
